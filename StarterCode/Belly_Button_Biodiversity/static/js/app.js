...
const firstFilter = (selectedData) => {
  return selectedData.slice(0,10);
}
...
function buildCharts(sample) {
  ...
  // @TODO: Build a Pie Chart
   // HINT: You will need to use slice() to grab the top 10 sample_values,
    ...
    let pieChart = [{
      values: firstFilter(sampleValues),
      labels: firstFilter(otuIds),
      hovertext: firstFilter(otuLabels),
      hoverinfo: 'hovertext',
      type: 'pie'
    }];
...
function init() {
  ...
  var selecElements = d3.select("#selDataset");
